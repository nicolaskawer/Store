...
// Route to handle product fetching via POST request
app.post('/fetch-products', async (req, res) => {
  try {
    const products = await Product.find();
    const productsObject = { allProducts: products }; // Insert into a specific object
    res.status(200).json(productsObject); // Return the object to the client
  } catch (error) {
    res.status(400).json({ message: 'Error fetching products', error });
  }
});
